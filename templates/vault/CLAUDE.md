...
| `digests/YYYY-MM-DD.md` | ジョブ | 日次ダイジェスト（`type: record`）。朝の判断 UI。`- [ ] 🧪` に `[x]` で試す候補、`- [ ] 📣` に `[x]` で X 投稿待ちへ。3 日放置した 🧪 は暗黙の見送り |
...
- **lint**: ダイジェストの `^aw-xxxxxxxx` ブロック ID を消さない（回収に使う）。`backlog.md` の `## 候補` / `## 完了`、`outputs.md` の `## 投稿待ち` / `## 投稿済み` の見出しを変えない
